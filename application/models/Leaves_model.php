...
class Leaves_model extends CI_Model
{
    ...
    function disapprove($user_id, $data)
    {
        $this->db->where('user_id', $user_id);
        $this->db->where('status', 0);
    ...
    function get_leaves_list($workspace_id, $user_id, $user_detail = 'no')
    {
        ...
        foreach ($res as $row) {
            ...
            $profile = '';
            $tempRow['id'] = $row['id'];
            $tempRow['first_name'] = $row['first_name'] . ' ' . $row['last_name'];
            ...
            $approve = '';
            if (check_permissions("leave_requests", "create")) {
                $approve = ' <a class="dropdown-item has-icon leave-action-alert" href="' . base_url('leaves/approve/' . $row['user_id']) . '" data-id="' . $row['user_id'] . '"><i class="fas fa-check"></i>' . (!empty($this->lang->line('label_approve')) ? $this->lang->line('label_approve') : 'Approve') . '</a>
                <a class="dropdown-item has-icon leave-action-alert" href="' . base_url('leaves/disapprove/' . $row['user_id']) . '" data-id="' . $row['user_id'] . '"><i class="fas fa-times"></i>' . (!empty($this->lang->line('label_disapprove')) ? $this->lang->line('label_disapprove') : 'Disapprove') . '</a>';
            }
