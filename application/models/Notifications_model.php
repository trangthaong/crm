...
class Notifications_model extends CI_Model
{
    ...
    function get_notifications_list($user_id, $workspace_id)
    {
        ...
        foreach ($res as $row) {
            if ($row['type'] == 'event') {
                $action = '<a href="' . base_url('calendar/' . $row['type_id']) . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
            if ($row['type'] == 'announcement') {
                $action = '<a href="' . base_url('announcements/details/' . $row['type_id']) . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
            if ($row['type'] == 'project') {
                $action = '<a href="' . base_url('projects/details/' . $row['type_id']) . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
            if ($row['type'] == 'task') {
                $action = '<a href="' . base_url('projects/tasks/' . $row['type_id']) . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
            if ($row['type'] == 'birthday') {
                $action = '<a href="notifications/details/' . $row['id'] . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
            if ($row['type'] == 'work-anniversary') {
                $action = '<a href="notifications/details/' . $row['id'] . '" class="btn btn-light mr-2"><i class="fas fa-eye"></i>';
            }
    ...
    function send_notifications($data)
    {
        ...
        if (!empty($data['user_ids'])) {
            $web_registrationIDs = fetch_details("users", ['web_fcm !=' => "", "web_fcm !=" => "null"], 'web_fcm', 10000, 0, "id", "DESC", "id", explode(",", $data['user_ids']));
        } else {
            $web_registrationIDs = fetch_details("users", ['web_fcm !=' => "", "web_fcm !=" => "null"], 'web_fcm', 10000, 0);
        }
        ...
        $registrationIDs_chunks = array_chunk(array_column($web_registrationIDs, "web_fcm"), 1000);
