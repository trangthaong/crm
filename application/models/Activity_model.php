    ...
    function get_activity_list($user_id, $workspace_id)
    {
        ...
        foreach ($res as $row) {
            if ($row['activity'] == 'Created') {
                $activity = '<span class="badge badge-success">' . ($this->lang->line('label_created') ? $this->lang->line('label_created') : 'Created') . '</span>';
            }

            if ($row['activity'] == 'Updated') {
                $activity = '<span class="badge badge-info">' . ($this->lang->line('label_updated') ? $this->lang->line('label_updated') : 'Updated') . '</span>';
            }
            if ($row['activity'] == 'Deleted') {
                $activity = '<span class="badge badge-danger">' . ($this->lang->line('label_deleted') ? $this->lang->line('label_deleted') : 'Deleted') . '</span>';
            }
            if ($row['activity'] == 'Uploaded') {
                $activity = '<span class="badge badge-warning">' . ($this->lang->line('label_uploaded') ? $this->lang->line('label_uploaded') : 'Uploaded') . '</span>';
            }
