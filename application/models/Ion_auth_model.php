...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, lang')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
		...
		if ($query->num_rows() === 1) {
			$user = $query->row();
			$query =  $this->db->select('g.id,g.name as group_name, ug.member_permissions,ug.client_permissions')
			->from('groups g')
	...
	public function set_session($user)
	{
		...
		$role = $this->users_model->get_role_by_user_id($user->id);
		$session_data = [
	...
	protected function _password_verify_sha1_legacy($identity, $password, $hashed_password_db)
	{
		...
		if ($this->config->item('store_salt', 'ion_auth')) {
			...
			$salt = mb_substr($hashed_password_db, 0, $salt_length);

			$hashed_password =  $salt . mb_substr(sha1($salt . $password), 0, -$salt_length);
		}
	...
	public function set_session_data($groups)
    {
        $this->trigger_events('pre_set_session');
