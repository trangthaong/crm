...
class Payments extends CI_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->load->database();
		$this->load->model(['workspace_model', 'payments_model', 'notifications_model', 'projects_model', 'users_model', 'invoices_model']);
	...
	public function create_payment()
	{
		...
		$this->form_validation->set_rules('payment_mode_id', str_replace(':', '', 'Payment mode is empty.'), 'trim|required|xss_clean');
		$this->form_validation->set_rules('amount', str_replace(':', '', 'Amount is empty.'), 'trim|required|xss_clean');
		$this->form_validation->set_rules('payment_date', str_replace(':', '', 'Payment date is empty.'), 'trim|required|xss_clean');
		...
		if ($this->form_validation->run() === TRUE) {
			...
			$data = array(
				...
				'note' => strip_tags((string)$this->input->post('note', true)),
	...
	public function get_payments_list()
	{
		if (!$this->ion_auth->logged_in()) {
			...
			$workspace_id = $this->session->userdata('workspace_id');
			return $this->payments_model->get_payments_list($workspace_id);
		}
	}
	...
	public function payment_modes()
	{
		if (!$this->ion_auth->logged_in()) {
			...
			if (!empty($workspace_id)) {
                ...
                $data['projects'] = $projects;
				$data['notifications'] = !empty($workspace_id) ? $this->notifications_model->get_notifications($this->session->userdata['user_id'], $workspace_id) : array();
				$this->load->view('payment-modes', $data);
			} else {
				redirect('home', 'refresh');
			}
		}
	...
	public function get_payment_mode_list()
	{
		if (!$this->ion_auth->logged_in()) {
			...
			$workspace_id = $this->session->userdata('workspace_id');
			return $this->payments_model->get_payment_mode_list($workspace_id);
		}
	}
	...
	public function delete_payment_mode()
	{
		...
		$payment_mode_id = $this->uri->segment(3);
		...
		redirect('payments/payment_modes', 'refresh');
	}
